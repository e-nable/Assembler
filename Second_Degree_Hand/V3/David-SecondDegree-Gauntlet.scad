...
Parametric Mechen - Second Degree - Gauntlet Portion
By David Orgeman
...
Gauntlet_Wall_Thickness=2.8;//default value 2.8
...
Velcro_Offset1=-1;//default value 0
...
Rail_Thickness=3.8;//default value 3.8
...
Tuner_Distance=83.7;//default value 90.5
...
//Print_Gauntlet determines if the gauntlet should be printed.  This is so that the
//one file can be used to print just the tuners if wanted by making the gauntlet false.
Print_Gauntlet=true;//default value true
...
//If springs will be used on the back of the tuner rods, then the rods and the
//block need to be slightly longer to make sure that the rods don't fall out if
//the springs are fully compressed.  The springs greatly improve hand function, so
//are strongly recommended.  Spring_Length_Adjustment is the extra length for
//this purpose.  The default values allow 10mm of tuner adjustment, and 10mm of
//spring compression without losing the rods.  Set this to 0 if springs will not
//be used.
Spring_Length_Adjustment=3.0;//default value 3.0

//If tuner springs will be used, then it is a good idea to make the tuner body
//shell longer to cover the screws and springs.  This keeps them from catching on
//clothing and other things.  Spring_Shield_Adjustment is the length of the shield.
//set this to 0 if you do not want a shield.
Spring_Shield_Adjustment=19.0;//default value 19.0
...
//the slots.
Tuner_Fit=0.25;//default value 0.25
...
if (Print_Tuners)
	...
	if (Tuner_Count>0)
		...
		for(i=[1:Tuner_Count])
			{
			translate([-15-3*Rod_Radius*i+20, 0, 0])
				tuner();
			}
		}
	}
...
//Rails that will hold the velcro and that attach on the outside of the wrist gimble						
					translate([3.5, Wrist_Width/(2*Scale_Factor)+Extra_Gap+3.5*tan(AngleW), 0])
						rotate(a=[0, 0, AngleW])				
							cube([Shell_Length/Scale_Factor-3.5, Rail_Thickness, 2*Hinge_Bearing_Radius]);
					translate([3.5, -Wrist_Width/(2*Scale_Factor)-Extra_Gap-Rail_Thickness-3.5*tan(AngleW), 0])
						rotate(a=[0, 0, -AngleW])				
							cube([Shell_Length/Scale_Factor-3.5, Rail_Thickness, 2*Hinge_Bearing_Radius]);
//Straight sections of rail to separate the gauntlet from the wrist pivot
					translate([-Rail_Extension/Scale_Factor, Pivot_Width/(2*Scale_Factor)+Extra_Gap, 0])
						cube([Rail_Extension/Scale_Factor+0.1, Rail_Thickness, 2*Hinge_Bearing_Radius]);
					translate([-Rail_Extension/Scale_Factor, -Pivot_Width/(2*Scale_Factor)-Extra_Gap-Rail_Thickness, 0])
						cube([Rail_Extension/Scale_Factor+0.1, Rail_Thickness, 2*Hinge_Bearing_Radius]);

//Widen from the wrist width to the pivot width with slanted rails
					hull()
						{
						translate([3.5, Wrist_Width/(2*Scale_Factor)+Extra_Gap+3.5*tan(AngleW), 0])
							rotate(a=[0, 0, AngleW])				
								cube([0.1, Rail_Thickness, 2*Hinge_Bearing_Radius]);
						translate([-0.1, Pivot_Width/(2*Scale_Factor)+Extra_Gap, 0])
							cube([0.1, Rail_Thickness, 2*Hinge_Bearing_Radius]);
						}
					hull()
						{
						translate([3.5, -Wrist_Width/(2*Scale_Factor)-Extra_Gap-3.5*tan(AngleW)-Rail_Thickness, 0])
							rotate(a=[0, 0, -AngleW])				
								cube([0.1, Rail_Thickness, 2*Hinge_Bearing_Radius]);
						translate([-0.1, -Pivot_Width/(2*Scale_Factor)-Extra_Gap-Rail_Thickness, 0])
							cube([0.1, Rail_Thickness, 2*Hinge_Bearing_Radius]);
						}

//Bearing surface for wrist extension and flexion
					translate([-Rail_Extension/Scale_Factor, Pivot_Width/(2*Scale_Factor)+Extra_Gap, Hinge_Bearing_Radius])
						rotate(a=[-90, 0, 0])
							cylinder(h=Rail_Thickness, r=Hinge_Bearing_Radius);
					translate([-Rail_Extension/Scale_Factor, -Pivot_Width/(2*Scale_Factor)-Extra_Gap-Rail_Thickness, Hinge_Bearing_Radius])
						rotate(a=[-90, 0, 0])
							cylinder(h=Rail_Thickness, r=Hinge_Bearing_Radius);
					}
...
//Round the back end of the beams that otherwise might poke the arm				
			...
			if (Include_Rail_Support)
				{
				translate([-Rail_Extension/Scale_Factor-0.65*Hinge_Bearing_Radius, Pivot_Width/(2*Scale_Factor)+Extra_Gap+0.01, 0])
					cube([0.65*Hinge_Bearing_Radius, Rail_Thickness-0.02, Hinge_Bearing_Radius/2]);
				translate([-Rail_Extension/Scale_Factor-0.65*Hinge_Bearing_Radius, -Pivot_Width/(2*Scale_Factor)-Extra_Gap-Rail_Thickness-0.01, 0])
					cube([0.65*Hinge_Bearing_Radius, Rail_Thickness-0.02, Hinge_Bearing_Radius/2]);
				translate([Shell_Length/Scale_Factor-Hinge_Bearing_Radius, (Shell_Length/Scale_Factor-Hinge_Bearing_Radius)*tan(AngleW)+Wrist_Width/(2*Scale_Factor)+Extra_Gap, 0])
					rotate(a=[0, 0, AngleW])	
						cube([0.75*Hinge_Bearing_Radius, Rail_Thickness-0.02, Hinge_Bearing_Radius/2]);
				translate([Shell_Length/Scale_Factor-Hinge_Bearing_Radius, -(Shell_Length/Scale_Factor-Hinge_Bearing_Radius)*tan(AngleW)-Wrist_Width/(2*Scale_Factor)-Extra_Gap-Rail_Thickness, 0])
					rotate(a=[0, 0, -AngleW])	
						cube([0.75*Hinge_Bearing_Radius, Rail_Thickness-0.02, Hinge_Bearing_Radius/2]);
				}
			}
...
//Holes for the wrist pivot screws
	translate([-Rail_Extension, 10, Hinge_Bearing_Radius*Scale_Factor])
		rotate(a=[-90, 0, 0])
			cylinder(h=60, r=Pivot_Screw_Size/2+0.2);
...
//	translate([0, -(Rod_Length+3), 0])
	difference()
		{
		translate([-2.3*Rod_Radius*(Tuner_Count+1)/2, 0, -6])
			cube([4.6*Rod_Radius*(Tuner_Count+1)/2, Rod_Length+3+Spring_Length_Adjustment+Spring_Shield_Adjustment, 3.8*Rod_Radius+Extra_Tuner_Height+6]);
					...
					hull()
						...
						translate([0, 0, 0])
							scale([Wrist_Width+2*Extra_Gap+2*Rail_Thickness+2*Tuner_Distance*tan(AngleW), 1, Wrist_Thickness+2*Extra_Gap+2*Rail_Thickness+2*Tuner_Distance*tan(AngleT)])
								rotate(a=[90, 0, 0])
									cylinder(h=0.1, r=0.5, $fn=64);
						translate([0, Rod_Length+8+Spring_Length_Adjustment+Spring_Shield_Adjustment, 0])
							scale([Wrist_Width+2*Extra_Gap+2*Rail_Thickness+2*(Tuner_Distance+Rod_Length+8+Spring_Length_Adjustment+Spring_Shield_Adjustment)*tan(AngleW), 1, Wrist_Thickness+2*Extra_Gap+2*Rail_Thickness+2*(Tuner_Distance+Rod_Length+8+Spring_Length_Adjustment+Spring_Shield_Adjustment)*tan(AngleT)])
								rotate(a=[90, 0, 0])
									cylinder(h=0.1, r=0.5, $fn=64);
						}	
	...
	difference()
		...
		for(i=[1:String_Count])
			...
			translate([-(String_Count*String_Hole_Diameter+(String_Count+1)*String_Hole_Spacing)/2+i*String_Hole_Spacing+(2*i-1)*String_Hole_Diameter/2, 50, 1.5*String_Hole_Spacing+0.5*String_Hole_Diameter+Extra_String_Height])
				rotate(a=[0, 30, 0])
				rotate(a=[90, 0, 0])
					cylinder(h=100, r=(String_Hole_Diameter/cos(30))/2, $fn=6);
			}
