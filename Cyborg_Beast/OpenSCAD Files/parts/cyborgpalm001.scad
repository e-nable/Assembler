module cyborgbeastpalm(palmLen=54)
	{
	difference()
		{
		difference()
...
module hardwarecutouts(palmLen)
	...
	//	Knuckle block hinge
		translate([0, palmLen/2 /* 27 /**/,5]) rotate([0,90,0]) cylinder(r=4/2, h=100, center=true, $fn=fn/2);
	//	Wrist hinges
		translate([0,-palmLen/2 /*27*/,5.5]) rotate([0,90,0]) cylinder(r=4/2, h=100, center=true, $fn=fn/2);
	...
	//	Elastic return cutout
		translate([31,-13,16]) rotate([-72,0,0]) cylinder(r=3/2, h=30, center=true, $fn=fn/2);
...
module cyborgbeast07palm(palmLen)
	...
	hull() 
		...
		//	Knuckles
		translate([0,palmLen/2 /* 27 */,knuckleR]) rotate([0,90,0]) 
		...
		//	Wrist
		for(i=[-1,1]) 
			translate([((palmLen/2)-0.4) /*26.6*/ *i,-12,wristH/2]) cube([th,10,wristH], center=true); 
		//	Back of wrist
		translate([0,-18,0]) 
			{
	...
	//	Wrist hinge
	for(i=[-1,1]) 
		translate([((palmLen/2)-0.4) /*26.6*/ *i,-palmLen/2 /*12*/+15,wristH/2]) { 
			cube([th,30,wristH], center=true); 
			translate([0,-palmLen/2 /*30*/ +wristH*1.5,0]) rotate([0,90,0]) 
